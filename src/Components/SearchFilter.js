...
class SearchFilter extends Component {
    ...
    render() {
        ...
        const userDataFilter = (row) => {
            ...
            return row.first_name.toLowerCase().includes(keyword) || row.last_name.toLowerCase().includes(keyword) ||
        ...
        const tablerows_elm = tablerows.map((row, index) => {
            ...
            return (
                <tr key={row.id}>
                    ...
                    {newFirstName ? <td dangerouslySetInnerHTML={{__html: newFirstName}}></td> :
                        <td> {row.first_name} </td>}
                     {newLastName ? <td dangerouslySetInnerHTML={{__html: newLastName}}></td> :
                        <td> {row.last_name} </td>}
                     {newEmail ? <td dangerouslySetInnerHTML={{__html: newEmail}}></td> :
                        <td> {row.email} </td>}
        ...
        return (
            ...
            <div>
                <div className="form-group">
                    ...
                    <small id="emailHelp" className="form-text text-muted"> search for people's names or emails
                        .</small>
                </div>
