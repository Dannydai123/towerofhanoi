...
class Timer extends Component {
  ...
  handleClickStartStop = () => {  //start timer for a timer object



    this.setState({lastStartTime: this.state.lastStartTime ? null : Date.now()})
    ...
    const origintimer = this.state.timer
        ...
        this.tickid = setInterval(() => {
            ...
            this.setState({timer :   origintimer +
...
handleUpdate = (e) => {
  ...
  componentDidMount() {
    ...
    render() {
        ...
        let btnclassname = this.state.isCounting? "btn-outline-danger w-100 btn" : " btn-outline-primary w-100 btn"
        ...
        return (
          <div className="card text-left w-50 mx-auto mb-2 ">
            ...
            <div className="card-body">
              <p className="card-title text-center h3 text-muted">{hour.toString().padStart(2,0)}:{minute.toString().padStart(2,0)}:{second.toString().padStart(2,0)}</p>
              <p className="card-text text-right">
                <a className="btn p-0" onClick={(e) => this.props.deletetimer(e, timer.id) }>
               ...
               <FontAwesomeIcon icon={faPenSquare} /> </a>
